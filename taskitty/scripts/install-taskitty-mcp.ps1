...
$settings = if (Test-Path -LiteralPath $ClineSettingsPath -PathType Leaf) { Get-Content -LiteralPath $ClineSettingsPath -Raw | ConvertFrom-Json } else { [pscustomobject]@{ mcpServers = [pscustomobject]@{} } }
if ($null -eq $settings.mcpServers) { $settings | Add-Member -NotePropertyName mcpServers -NotePropertyValue ([pscustomobject]@{}) }
$settings.mcpServers | Add-Member -NotePropertyName taskitty -NotePropertyValue ([pscustomobject]@{ type = "stdio"; command = "taskitty-mcp"; args = @(); timeout = 300; disabled = $false }) -Force
[IO.File]::WriteAllText($ClineSettingsPath, ($settings | ConvertTo-Json -Depth 20), [Text.UTF8Encoding]::new($false))
[pscustomobject]@{ command = "taskitty-mcp"; npmBin = $npmBin; clineSettings = $ClineSettingsPath; restartRequired = $true } | ConvertTo-Json
